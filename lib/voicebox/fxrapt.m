...
lagval=NaN(nframe,ncands-1);    % lag of each voiced candidate
...
vix=find(best>1);
fx=NaN(nframe,1);                        % unvoiced frames will be NaN
...
if ~nargout || any(mode=='g')
    ...
    blag=NaN(nframe,1);                        % unvoiced frames will be NaN
    ...
    sup=NaN(ns,1);               % unprocessed speech - plot in black
    sup(tsa)=s(tsa);
